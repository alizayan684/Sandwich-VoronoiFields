...
def drawPath(predecessor, currentNode, draw):
    ...
    while currentNode in predecessor:
        ...
        total_dist += heuristicFunction(temp.getPosition(), currentNode.getPosition())
...
def AStarAlgorithm(draw, grid, startNode, endNode):
    ...
    while not openSet.empty():
        ...
        if current == endNode:
            total_distance = drawPath(predecessor, endNode, draw)
            endNode.makeEnd()
            print(f"The End Node is {total_distance} units away from the Source Node")
            return True
